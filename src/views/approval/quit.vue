...
export default {
  ...
  methods: {
    ...
    async getApprovalsTaskDetail() {
      const resd = await getApprovalsTaskDetail(this.approvalId)
      this.taskInstanceOutList = resd.data.data
    },
    // 图片下载
    async getReviewHistory(id) {
      const rese = await downImg(id)
      const response = rese.data.data
