...
export default {
  ...
  methods: {
    ...
    async  getEmployeeList() {
      ...
      // this.loading = true
      const res = await getEmployeeList({ ...this.page, ... this.formData })
    ...
    async  changeSalary(key, userId) {
      ...
      const res = await getSalaryDetail(userId)
      this.selectedSalaryInfo = res.data.data
    },
    fixedSalary(key) {
      this.topLabel = '定薪'
      this.currentComponent = key
